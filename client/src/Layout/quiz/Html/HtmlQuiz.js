...
class HtmlQuiz extends Component {
  constructor (props) {
    super(props);
  ...
  render () {
    ...
    const HtmlQuestionCount = ReactQust.map(data => data.HtmlMcq.length);
    return (
      <div>
        <Navbar />
        <div className='quiz_main'>
          <h2>HTML Questions & Answers(Questions {HtmlQuestionCount})</h2>
          <hr />
        </div>
        <div className='quiz_data'>
          {ReactQust.map((data, index) => (
            <div key={index} className='quiz_question_min'>
              {data.HtmlMcq.slice(0, visible).map((item, index) => (
