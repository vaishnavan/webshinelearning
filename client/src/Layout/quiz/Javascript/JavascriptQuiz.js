...
class JavascriptQuiz extends Component {
  constructor (props) {
    super(props);
  ...
  render () {
    ...
    const JsQuestionCount = ReactQust.map(data => data.JsMcq.length);
    return (
      <div>
        <Navbar />
        <div className='quiz_main'>
          <h2>JavaScript Questions & Answers (Questions {JsQuestionCount})</h2>
          <hr />
        </div>
        <div className='quiz_data'>
          {ReactQust.map((data, index) => (
            <div key={index} className='quiz_question_min'>
              {data.JsMcq.slice(0, visible).map((item, index) => (
