...
class ReactQuiz extends Component {
  ...
  render () {
    ...
    const ReactQuestionCount = ReactQust.map(data => data.ReactMcq.length);
    // console.log(ReactQuestionCount);
    return (
      <div>
        <Navbar />
        <div className='quiz_main'>
          <h2>React Questions & Answers (Question {ReactQuestionCount})</h2>
          <hr />
        </div>
        <div className='quiz_data'>
          {ReactQust.map((data, index) => (
            <div key={index} className='quiz_question_min'>
              {data.ReactMcq.slice(0, visible).map((item, index) => (
